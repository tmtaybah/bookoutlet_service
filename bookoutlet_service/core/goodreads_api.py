...
# returns user(id, name) tuple from goodreads get user api call takes oauth session
def get_user(session):

    response = session.get(base_url + 'api/auth_user', header_auth=True)

    if response.status_code != 200:
        raise Exception('Could not get user ID -- Error {}'
...
def get_page(page_num, member_id, shelf):
    ...
    params = {'id': member_id, 'shelf': shelf, 'key': developer_key,
              'per_page': 200, 'page': page_num}
...
def get_books(root):
    ...
    for book in book_nodes:
        ...
        book = GoodreadsBook(title=title, title_series=title_series,
                             author=author)
        books += [book]
