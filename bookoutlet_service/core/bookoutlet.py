...
def get_bestsellers():
    ...
    # step 1 - get page by creating repsponse object
    params = {'size': 200}
    response = get(top200_url, params=params)
    print(response)

    # step 2 - create soup object so we can begin grabbing items off the page
    soup = BeautifulSoup(response.content, 'html.parser')

    # step 3 - grab book items
...
def get_book(book_item):
    ...
    book = BookoutletBook(title=title, author=author, price=price,
                          format=format, scratch_n_dent=scratch_n_dent, link=link)
...
if __name__ == '__main__':
    ...
    goodreads_book = GoodreadsBook(title="Shadowcaster", title_series="Shattered Realms",
